...
qa[85] = ["ｊｓで文字列に使うのはどれでしょう",
            "　’＝シングルコーテーション",
              "85_ダブルクォーテーション”も同様に使えます。ただ、ダブルクォーテーション”は HTML 内で利用されることが多いので、干渉を避けるため JavaScript 内ではシングルクォーテーションを利用することが多い];
...
qa[82] = ["ＨＴＭＬでＹｏｕＴｕｂｅ動画の埋め込みに使われるタグはどれでしょう",
            "ｓｃｒｉｐｔタグ",
              "82_<script　src=" "> </script>　src属性=source(ｿｰｽ=出典・由来)];
...
qa[79] = ["ＨＴＭＬのリストタグはどれでしょう",
            "ｕｌタグ＝unordered list(ｱﾝｵｰﾀﾞｰﾄﾞ･ﾘｽﾄ)、ｏｌタグ＝ordered list（ｵｰﾀﾞｰﾄﾞ･ﾘｽﾄ）",
              "79_<ul> <li>　</li> <ul>。　li(ｴﾙｱｲ)=list item(ﾘｽﾄｱｲﾃﾑ=項目)。liタグはulタグやolタグの子要素にしなければならない。];

qa[78] = ["ＨＴＭＬの画像タグはどれでしょう",
            "ｉｍａｇｅタグ＝image（イメージ）",
              "78_<img src:"https:～～ " alt=" " />。src属性=source(ｿｰｽ=出典・由来)  alt属性=alternate(ｵﾙﾀﾈｰﾄ=代替)];

qa[77] = ["<a href="https:～～ ">　</a>の要素と属性はどれでしょう",
            "ａタグ＝要素　ｈｒｅｆ＝属性",
              "77_href 属性は、ハイパーリンクのリンク先をａタグ要素に情報を与えることができます。なお、href属性はウェブサイトへリンク。src属性はファイルの場所を指定します];

qa[76] = ["ＨＴＭＬのリンクタグはどれでしょう",
            "ａタグ＝anchor(アンカー=船のいかり)",
              "76_<a><／a>。なお、リンク先は<a href="https:～～ ">。href=hypertext reference(ﾊｲﾊﾟｰﾃｷｽﾄﾘﾌｧﾚﾝｽ=ﾊｲﾊﾟｰﾃｷｽﾄの参照)];

qa[75] = ["ＨＴＭＬの段落タグはどれでしょう",
            "ｐタグ＝paragraph(ﾊﾟﾗｸﾞﾗﾌ=段落)",
              "75_<p><／p>。なお、<br>タグを文面に入れることで改行できます。br=break(ﾌﾞﾚｰｸ=改行)"];

qa[74] = ["ＨＴＭＬの見出しタグはどれでしょう",
            "ｈタグ＝header(ﾍｯﾀﾞｰ=見出し)",
              "74_<h1><／h1>。＜Body＞＜／Body＞タグ内に入力します。なお、h1～h6まであります"];
