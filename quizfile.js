...
qa[115] = ["HTTPをSSLやTLSという方式で暗号し通信を行うプロトコルは",
          "HTTPS",
...
qa[83] = ["Linuxにおいてﾌｧｲﾙやﾃﾞｨﾚｸﾄﾘを削除するｺﾏﾝﾄﾞは",
          "rm=ReMove(ﾘﾑｰﾌﾞ)",
...
qa[78] = ["Linuxのｺﾏﾝﾄﾞで現在のﾃﾞｨﾚｸﾄﾘのﾌｧｲﾙとﾃﾞｨﾚｸﾄﾘを表示するものは",
          "ls=List(ﾘｽﾄ)",
          "78_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[77] = ["Linuxのｺﾏﾝﾄﾞで現在のﾃﾞｨﾚｸﾄﾘから別のﾃﾞｨﾚｸﾄﾘに移動するものは",
          "cd=Change Directory(ﾁｪﾝｼﾞﾃﾞｨﾚｸﾄﾘ)",
          "77_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[76] = ["Linuxのｺﾏﾝﾄﾞで１つ上のﾃﾞｨﾚｸﾄﾘに移動するものは",
          "cd ../",
          "76_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[75] = ["Linuxのｺﾏﾝﾄﾞでﾙｰﾄﾃﾞｨﾚｸﾄﾘに移動するものは",
          "cd /",
          "75_23/94.02-04.ﾌｧｲﾙ操作_ﾙｰﾄﾃﾞｨﾚｸﾄﾘ=一番上の階層"];

qa[74] = ["Linuxのｺﾏﾝﾄﾞでﾎｰﾑﾃﾞｨﾚｸﾄﾘに移動するものは",
          "cd ~",
...
qa[71] = ["Linuxでﾃﾞｨﾚｸﾄﾘやﾌｧｲﾙを削除するｺﾏﾝﾄﾞは",
          "rm=ReMove(ﾘﾑｰﾌﾞ)",
          "71_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[70] = ["Linuxのｺﾏﾝﾄﾞで〇〇ﾃﾞｨﾚｸﾄﾘを削除するためのｺｰﾄﾞは",
          "rm -r",
          "70_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[69] = ["Linuxのｺﾏﾝﾄﾞでファイルやﾃﾞｨﾚｸﾄﾘをコピーするものは",
          "cp=Copy(ｺﾋﾟｰ)",
          "69_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[68] = ["Linuxのｺﾏﾝﾄﾞで中身を含めてﾃﾞｨﾚｸﾄﾘをｺﾋﾟｰするものは",
          "cp -r",
          "68_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[67] = ["Linuxのｺﾏﾝﾄﾞでﾌｧｲﾙやﾃﾞｨﾚｸﾄﾘを移動するものは",
          "mv=Move(ﾑｰﾌﾞ)",
          "67_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[66] = ["Linuxのｺﾏﾝﾄﾞで[mv b c]で移動先にﾃﾞｨﾚｸﾄﾘがない場合の処理は",
          "ﾃﾞｨﾚｸﾄﾘbが移動し、ﾃﾞｨﾚｸﾄﾘcに名前が変わる",
          "66_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[65] = ["Linuxのｺﾏﾝﾄﾞで[mv b c]で移動先にﾃﾞｨﾚｸﾄﾘcがある場合の処理は",
         "ﾃﾞｨﾚｸﾄﾘｃの中に、ﾃﾞｨﾚｸﾄﾘｂが移動する",
         "65_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[64] = ["Linuxのｺﾏﾝﾄﾞ[mv b c]で移動先ﾃﾞｨﾚｸﾄﾘがc/bである場合の処理は",
         "ﾃﾞｨﾚｸﾄﾘｃが重複するためｴﾗｰ表示がでる",
         "64_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[63] = ["Linuxのｺﾏﾝﾄﾞ[mv tmp/c workspace/tmp]でﾃﾞｨﾚｸﾄﾘがworkspace/tmpまでの場合の処理は",
         "ﾃﾞｨﾚｸﾄﾘworkspace/tmpの中に、ﾃﾞｨﾚｸﾄﾘcが移動される",
         "63_23/94.02-04.ﾌｧｲﾙ操作_"];

qa[62] = ["Vagrantfileの説明をしているものは",
...
qa[9] = ["HTMLでYouTubeｅ動画の埋め込みに使われるﾀｸﾞは",
          "iframeﾀｸﾞ（ｱｲﾌﾚｰﾑ）",
          "9_05/94.01-05.HTMLで作る自己紹介ﾍﾟｰｼﾞ_"];

qa[8] = ["JavaScriptで文字列に使うのは",
          "ｼﾝｸﾞﾙｺｰﾃｰｼｮﾝ(&#039;)",
